...
static int set_segment_filename(AVFormatContext *s)
{
    ...
    //snprintf(insert_cmd, sizeof(insert_cmd), "/usr/local/bin/nvr/scheduller.sh test %s %s %d %s", seg->list, seg->cur_entry.filename, seg->segment_idx, av_basename(oc->filename));
    //printf("%s\n",insert_cmd);
    //FILE *pfp = popen(insert_cmd, "r");
    //pclose(pfp);
	...
	//MYCODE
   seg->cur_entry.start_clock = time(0);
...
{
    SegmentContext *seg = s->priv_data;
    //printf("segment_list_open");
    int ret;

    snprintf(seg->temp_list_filename, sizeof(seg->temp_list_filename), seg->use_rename ? "%s" : "%s", seg->list);
...
static void segment_list_print_entry(AVIOContext      *list_ioctx,
                                     ...
                                     void *log_ctx)
{
    //printf("segment_list_print_entry\n");
    switch (list_type) {
    case LIST_TYPE_FLAT:
...
static int segment_end(AVFormatContext *s, int write_trailer, int is_last)
{
    ...
    //char insert_cmd[1024] = {0, };
    //snprintf(insert_cmd, sizeof(insert_cmd), "/usr/local/bin/nvr/scheduller.sh newrec %s %s", seg->list, seg->cur_entry.filename);
    //FILE *pfp = popen(insert_cmd, "r");
    //pclose(pfp);
    //memset(insert_cmd, 0, sizeof(insert_cmd));
//	av_log(s, AV_LOG_ERROR, "The segment list current entry duration: %f\n", seg->cur_entry.end_time - seg->cur_entry.start_time);
    //snprintf(insert_cmd, sizeof(insert_cmd), "/usr/local/bin/nvr/scheduller.sh duration %s %s %f", seg->list, seg->cur_entry.filename, seg->cur_entry.end_time - seg->cur_entry.start_time);
    //FILE *duration_pfp = popen(insert_cmd, "r");
    //pclose(duration_pfp);


    if (seg->list) {
	/*if (seg->list_size || seg->list_type == LIST_TYPE_M3U8)
		printf("Segment List type is m3u8\n");
	else    printf("Segment List type is not m3u8\n");*/
        if (seg->list_size || seg->list_type == LIST_TYPE_M3U8) {
            ...
            /* drop first item */
	    //printf("seg->list_size -%d ,seg->segment_count -%d\n",seg->list_size , seg->segment_count);
		...
		//MYCODE
		struct tm *loctime;
		loctime = localtime (&entry->start_clock);
		char stm[100];
                strftime (stm, 100 , "%Y-%m-%d %H:%M:%S", loctime);
		//printf("Need delete entry->filename = %s , duration %f, camName %s , StartTime is '%s'\n", 
			//entry->filename, entry->end_time - entry->start_time,seg->list, stm);
                char insert_cmd[1024] = {0, };
                snprintf(insert_cmd, sizeof(insert_cmd), "/usr/local/bin/nvr/scheduller.sh %s %s %f '%s'",
			seg->list, entry->filename, entry->end_time - entry->start_time, stm);
		FILE *cpfp = popen(insert_cmd, "r");
                pclose(cpfp);
...
{
    //printf("select_reference_stream\n");
    SegmentContext *seg = s->priv_data;
    int ret, i;
